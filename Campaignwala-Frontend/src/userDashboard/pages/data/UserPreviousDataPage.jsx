...
const UserPreviousDataPage = ({ darkMode, setDarkMode }) => {
  ...
  const fetchPreviousData = async () => {
    setLoading(true);
    try {
      console.log('Fetching previous assigned data...');
      
      // Get user's previous data (assigned before today)
      const dataResult = await dataService.getUserPreviousData({
        page: 1,
        limit: 1000
      });
      console.log('Previous data result:', dataResult);
      
      if (dataResult.success) {
        const previousList = dataResult.data?.data || [];
        console.log('Previous data list:', previousList);
        
        // Process the data to extract user-specific assignment info
        const processedData = previousList.map(item => {
        ...
        console.error('Failed to fetch previous data:', dataResult.error);
        setPreviousData([]);
        setPreviousCounts({ total: 0, pending: 0, called: 0, closed: 0 });
      }
    } catch (error) {
  ...
  const selectAllData = () => {
    const filteredData = filterPreviousData();
    ...
    try {
      ...
      if (updateResult.success) {
        ...
        setTimeout(() => {
          ...
          const item = previousData.find(d => d._id === dataId);
      ...
      <div className="p-3 sm:p-4 md:p-6">
        ...
        <div className="hidden md:block mb-8">
          <div className="flex justify-between items-start md:items-center mb-4">
            <div>
              <h1 className="text-2xl font-bold mb-2">Previous Assigned Data</h1>
              <p className={`${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>Manage data assigned before today</p>
            </div>
          ...
          <div className={`rounded-lg sm:rounded-xl shadow-sm sm:shadow p-2.5 sm:p-3 md:p-4 ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
            <div className="flex items-center">
              ...
              <div className="min-w-0 flex-1">
                <p className={`text-[10px] sm:text-xs md:text-sm truncate ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>Total Previous</p>
                <p className="text-base sm:text-lg md:text-2xl font-bold truncate">{previousCounts.total}</p>
              </div>
            </div>
          </div>
          ...
          <div className={`rounded-lg sm:rounded-xl shadow-sm sm:shadow p-2.5 sm:p-3 md:p-4 ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
            <div className="flex items-center">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-base sm:text-lg md:text-2xl font-bold truncate">{previousCounts.pending}</p>
              </div>
            </div>
          </div>
          ...
          <div className={`rounded-lg sm:rounded-xl shadow-sm sm:shadow p-2.5 sm:p-3 md:p-4 ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
            <div className="flex items-center">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-base sm:text-lg md:text-2xl font-bold truncate">{previousCounts.called}</p>
              </div>
            </div>
          </div>
          ...
          <div className={`rounded-lg sm:rounded-xl shadow-sm sm:shadow p-2.5 sm:p-3 md:p-4 ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
            <div className="flex items-center">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-base sm:text-lg md:text-2xl font-bold truncate">{previousCounts.closed}</p>
              </div>
            </div>
          </div>
        </div>
        
        {/* Mobile Filters Overlay - Same as UserTodayDataPage */}
        {showMobileFilters && (
        ...
        {/* Data Table - Reuse same structure as UserTodayDataPage */}
        <div className={`rounded-xl shadow overflow-hidden ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
          <div className={`p-4 md:p-6 border-b ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}>
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0">
              <div>
                <h2 className={`text-lg font-semibold ${darkMode ? 'text-gray-300' : 'text-gray-800'}`}>Previous Data List</h2>
                <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                  Showing {filteredData.length} of {previousData.length} records
                </p>
              </div>
          ...
          <div className="overflow-x-auto">
            {loading ? (
              <div className="text-center py-12">
                ...
                <p className={darkMode ? 'text-gray-400' : 'text-gray-500'}>Loading previous data...</p>
              </div>
            ) : filteredData.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                  {searchQuery || filterStatus !== 'all' 
                    ...
                    : 'No previous data assigned to you'}
                </p>
              </div>
            ) : (
                ...
                <div className="mb-4 md:mb-0">
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                    ...
                    <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs ${
                      ...
                      Called: {previousCounts.called}
                    </span>
                    <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs ${
                      darkMode ? 'bg-green-900/30 text-green-300' : 'bg-green-100 text-green-800'
                    }`}>
                      <CheckCircle size={12} className="mr-1" />
                      Closed: {previousCounts.closed}
                    </span>
                  </div>
                </div>
                
                <div className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                  <p>Data assigned before today</p>
                </div>
              </div>
            </div>
          )}
...
export default UserPreviousDataPage;
